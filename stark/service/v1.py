...
class Option(object):
    ...
    def get_queryset_or_tuple(self, model_class, request, *args, **kwargs):
        ...
        if isinstance(
                field_object,
                ForeignKey) or isinstance(
                field_object,
                ManyToManyField):
            # FK和M2M,应该去获取其关联表中的数据： QuerySet
            db_condition = self.get_db_condition(request, *args, **kwargs)
            return SearchGroupRow(
                title, field_object.remote_field.model.objects.filter(
                    **db_condition), self, request.GET)
        else:
            # 获取choice中的数据：元组
            self.is_choice = True
            return SearchGroupRow(
                title, field_object.choices, self, request.GET)
...
class StarkHandler(object):
    ...
    def display_checkbox(self, obj=None, is_header=None, *args, **kwargs):
        ...
        if is_header:
            return "选择"
        return mark_safe(
            '<input type="checkbox" name="pk" value="%s" />' %
            obj.pk)
    ...
    def changelist_view(self, request, *args, **kwargs):
        ...
        if isinstance(prev_queryset, bool):
        ...
        for option_object in search_group:
            row = option_object.get_queryset_or_tuple(
    ...
    def reverse_change_url(self, *args, **kwargs):
        ...
        return self.reverse_commons_url(
            self.get_change_url_name, *args, **kwargs)
    ...
    def reverse_delete_url(self, *args, **kwargs):
        ...
        """
        return self.reverse_commons_url(
            self.get_delete_url_name, *args, **kwargs)
...
class StarkSite(object):
    ...
    def register(self, model_class, handler_class=None, prev=None):
        ...
        self._registry.append({'model_class': model_class, 'handler': handler_class(
            self, model_class, prev), 'prev': prev})
    ...
    def get_urls(self):
        ...
        for item in self._registry:
            ...
            if prev:
                patterns.append(url(
                    r'^%s/%s/%s/' % (app_label, model_name, prev,), (handler.get_urls(), None, None)))
            else:
                patterns.append(
                    url(r'%s/%s/' % (app_label, model_name,), (handler.get_urls(), None, None)))
